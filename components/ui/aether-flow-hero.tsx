// @ts-nocheck — vendored 21st.dev component (aether-flow-hero)
"use client";
...
// --- Standalone background (canvas only), recolored brass for the cream theme ---
export const AetherFlowBackground: React.FC<{ className?: string }> = ({ className = "" }) => {
  ...
  React.useEffect(() => {
    ...
    class P {
      ...
      draw() {
        ...
        ctx!.fillStyle = "rgba(200, 169, 110, 0.85)"; // brass
        ctx!.fill();
      }
      update() {
    ...
    const init = () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        particles.push(new P(x, y, (Math.random() - 0.5) * 0.4, (Math.random() - 0.5) * 0.4, size));
      }
    };
      ...
      for (let a = 0; a < particles.length; a++) {
        for (let b = a; b < particles.length; b++) {
          ...
          if (d < (canvas.width / 8) * (canvas.height / 8)) {
            const op = 1 - d / 18000;
            if (op > 0) {
              ctx!.strokeStyle = `rgba(200, 169, 110, ${op * 0.35})`;
              ctx!.lineWidth = 1;
              ctx!.beginPath();
              ctx!.moveTo(particles[a].x, particles[a].y);
              ctx!.lineTo(particles[b].x, particles[b].y);
    ...
    const animate = () => {
      ...
      ctx.clearRect(0, 0, canvas.width, canvas.height); // transparent — cream shows through
